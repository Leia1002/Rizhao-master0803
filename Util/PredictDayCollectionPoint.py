...
class PredictDayCollectionPoint(object):
    def __init__(self):
    ...
    def getFromDB(self, region):
        ...
        try:
            curs = db.cursor()
            sqlx = 'SELECT JZDATE,SUM (PSGINCOUNT) FROM JZ_AREAPSGFLOW WHERE JZTYPE = 1 AND AREAID = \'' + region + '\' AND TO_CHAR (JZDATE, \'yyyy-mm-dd\') >= TO_CHAR (SYSDATE - 22, \'yyyy-mm-dd\') AND TO_CHAR (JZDATE, \'yyyy-mm-dd\') < TO_CHAR (SYSDATE, \'yyyy-mm-dd\') GROUP BY AREAID, JZDATE ORDER BY JZDATE DESC'
            curs.execute(sqlx)
