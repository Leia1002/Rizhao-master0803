...
class PredictHourEvacuatePassenger(object):
    def __init__(self):
    ...
    def getFromDB(self, region):
        ...
        try:
            curs = db.cursor()
            sqlx = 'select JZDATE, PSGOUTCOUNT from JZ_TRANRUNDATA where JZTYPE = 1  and TRANSTYPE = ' + region + 'and JZDATE >= SYSDATE - 21 ORDER BY JZDATE DESC '
            curs.execute(sqlx)
