...
class PredictHourRegion(object):
    def __init__(self):
    ...
    def getFromDB(self, region, inOrOut):
        ...
        try:
            curs = db.cursor()
            sqlx = 'SELECT JZDATE,SUM (+ inOrOut + ) FROM JZ_AREAPSGFLOW WHERE JZTYPE = 1 AND AREAID = \'' + region + '\' AND JZDATE >= SYSDATE - 21 GROUP BY AREAID, JZDATE ORDER BY JZDATE DESC'
            curs.execute(sqlx)
