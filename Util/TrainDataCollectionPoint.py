...
class TrainDataCollectionPoint(object):
    def __init__(self):
    ...
    def getFromDB(self, region):
        ...
        try:
            curs = db.cursor()
            sqlx = 'SELECT JZDATE,SUM (PSGINCOUNT) FROM JZ_AREAPSGFLOW WHERE JZTYPE = 1 AND CAMERAID = \'' + region + \
                   '\' AND TO_CHAR (JZDATE, \'yyyy-mm-dd\') >= TO_CHAR (SYSDATE - 90, \'yyyy-mm-dd\') AND TO_CHAR (JZDATE, \'yyyy-mm-dd\') < TO_CHAR (SYSDATE, \'yyyy-mm-dd\') GROUP BY CAMERAID, JZDATE ORDER BY JZDATE DESC'
            curs.execute(sqlx)
